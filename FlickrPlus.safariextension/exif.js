...
if( doc.querySelector( 'meta[name="medium"][content="image"]' ) && window === window.top ) {
	...
	var exif = {
		...
		, control:  doc.createDocumentFragment()

		, init: function( data ) {
				clog( 'Initializing an exif object' );
				clog( data );
				
				/** If there's no exif data available, there's nothing to d
				if( !data.Model ) {
					return false;
				}
				
				this.fragment = this.build_dom();
				this.anchor.appendChild( this.fragment );
				this.control  = this.display_control();
				
				this.control.addEventListener( 'click', flip );
		...
		, build_dom: function() {
				clog( '--> Building the exif fragment' );
				
				var img = this.anchor.firstElementChild;
			
				if( this.fragment.hasChildNodes() ) {
					clog( '<-- Fragment returned from cache' );
				...
				info.setAttribute( 'cellpadding', 0 );
				container.setAttribute( 'id', 'exif-wrapper' );
				addClass( container, 'face back' )
				
				/** Set the exif dimensions to match those of the image
				container.style.width  = img.offsetWidth + 'px';
				container.style.height = img.offsetHeight + 'px';
				
				clog( '----> Setting container size (' + parseInt( container.style.width ) + 'x' + parseInt( container.style.height ) + ') = image size (' + img.offsetWidth + 'x' + img.offsetHeight + ')' );
